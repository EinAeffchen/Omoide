...
const NoExifDatePage: React.FC = () => {
  ...
  const fetchItems = useCallback(
    async (cursor: string | null, append: boolean) => {
      ...
      setError(null);
      try {
        const page = await getNoExifDateMedia({
  ...
  const handleAction = async (action: NoExifDateResolveAction) => {
    if (selectedIds.size === 0) return;
    setIsActionLoading(true);
    try {
      const { removed } = await resolveNoExifDate(Array.from(selectedIds), action);
        ...
        >
          <Stack direction="row" spacing={2} alignItems="center" flexWrap="wrap">
            <Typography variant="subtitle1">
              {total.toLocaleString()} item{total !== 1 ? "s" : ""} without EXIF date
            </Typography>
